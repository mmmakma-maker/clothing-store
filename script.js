...
// ------------------ تسجيل دخول التاجر ------------------
const adminPasswordValue = "1234"; // كلمة مرور تجريبية
...
function renderProducts(list) {
  ...
  list.forEach(product => {
    ...
    if (isAdmin) {
      deleteButtonHTML = `<button onclick="deleteProduct(${product.id})" style="background:red;">حذف المنتج</button>`;
    }
...
function showProductDetail(id) {
  ...
  let colorOptions = "";
  product.colors.forEach(color => colorOptions += `<option value="${color}">${color}</option>`);

  let sizeOptions = "";
  product.sizes.forEach(size => sizeOptions += `<option value="${size}">${size}</option>`);

  const suggestedProducts = products.filter(p => p.category === product.category && p.id !== id);
  let suggestedHTML = "";
  if (suggestedProducts.length > 0) {
    suggestedHTML = `<h3>منتجات مقترحة</h3><div class="suggested-products">`;
  ...
  productDetailDiv.innerHTML = `
    ...
    <br>
    <button onclick="addDetailToCart(${id})" ${product.stock===0?'disabled':''}>إضافة للسلة</button>
    <br><br>
    <button onclick="backToProducts()">⬅ العودة للمنتجات</button>
    ${suggestedHTML}
  `;
}
...
function addDetailToCart(id) {
  ...
  if (quantity > product.stock) {
    alert(`لا يمكن إضافة أكثر من ${product.stock} قطع متوفرة في المخزن!`);
    return;
  }
...
function renderCartPopup() {
  ...
  cart.forEach((item, index) => {
    ...
    li.innerHTML = `
      ${item.name} (${item.selectedColor} / ${item.selectedSize}) - ${item.price} ريال
      <input type="number" value="1" min="1" style="width:40px" onchange="updateQuantity(${index}, this.value)">
...
function updateQuantity(index, value) {
  ...
  cart.splice(index, 1);
  for (let i = 0; i < value; i++) cart.splice(index, 0, product);
